...
-(void)dealloc {
    for (SCDragViewChild* child in [gridView.subviews arrayByAddingObjectsFromArray:dragView.subviews]) {
...
-(void)arrangeToGrid {
    int viewNum = 0;
    for (SCDragViewChild* subview in [[gridView.subviews arrayByAddingObjectsFromArray:dragView.subviews] sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"positionIndex" ascending:YES]]]) {
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([object isKindOfClass:[SCDragViewChild class]]) {
...
-(SCDragViewChild*)childAtPosition:(NSInteger)position {
    SCDragViewChild* retChild = nil;
    for (SCDragViewChild* child in [gridView.subviews arrayByAddingObjectsFromArray:dragView.subviews]) {
...
-(void)animateMoveChildsFromIndex:(NSInteger)startIndex toIndex:(NSInteger)stopIndex forSteps:(NSInteger)steps {
    ...
    if (dir > 0) {
        for (NSInteger i = startIndex; i <= stopIndex; i += dir) {
            SCDragViewChild* child = [self childAtPosition:i];
    ...
    for (SCDragViewChild* child in childs) {
        [self animateMoveChild:child toPos:child.positionIndex+steps];
    }
}
         
-(void)animateMoveChild:(SCDragViewChild*)child toPos:(NSInteger)newIndex {
...
-(id)childWithUserObject:(id)userObject {
    for (SCDragViewChild* child in [dragView.subviews arrayByAddingObjectsFromArray:gridView.subviews]) {
