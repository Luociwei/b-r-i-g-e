			...
			if (text_p < endChar_p) {
				...
				if (addCurrentRowAndStartNew) {
					...
					if (addCurrentRow) {
                        ...
                        if (dot_count %10==0)
                        {
                            [[NSNotificationCenter defaultCenter]postNotificationName:kNotificationIndicatorMsg object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@".",kStartupMsg,[NSNumber numberWithInt:MSG_LEVEL_NORMAL],kStartupLevel,[NSNumber numberWithFloat:100.0+dot_count/10], kStartupPercentage, nil]];
                        }
                        dot_count ++;
					}
