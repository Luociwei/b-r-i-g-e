...
def correlation_plot(table_data_x,table_data_y,forXorY):
    try:
        ...
        if select_color_by_left == 0 and select_color_by_right == 0:  #没有点击color by
            ...
            xValue=[]
            yValue=[]
            if tb_len_x>tb_len_y:  #删除空元素 和长度不匹配的，按照做小的长度为基准tmp = [i for i in v if (i !='' and dropUnFloat(i))]
                for i,v in enumerate(y_tb_value):
                    if x_tb_value[i]!='' and v!='' and dropUnFloat(x_tb_value[i]) and dropUnFloat(v):
                        xValue.append(x_tb_value[i])
                        yValue.append(v)

            else:
                for i,v in enumerate(x_tb_value):
                    if y_tb_value[i]!='' and v!='' and dropUnFloat(x_tb_value[i]) and dropUnFloat(v):
                        xValue.append(v)
...
def draw_correlation(xValue,yValue,x_item_name,y_item_name,pic_save_path,x_lsl,x_usl,y_lsl,y_usl,start_time_first,start_time_last):
    ...
    #fig, axes = plt.subplots( figsize=(8, 6), facecolor='#ccddef')
    print("????333-??----")
...
def correlation_coefficient_calc(data_l1,data_l2,item_name1,item_name2):
    ...
    print('correlation data:===>>>>',data.corr(method='pearson').values)
    print('correlation data:===>>>>',data.corr(method='spearman').values)
