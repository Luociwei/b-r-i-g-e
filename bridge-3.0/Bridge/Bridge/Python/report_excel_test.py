...
def send_progress_info(info,progress,title="Excel report"):
    pass
    #import json
...
def creat_excel_report_file(path,file,cpk_lsl,cpk_usl,event,fail_plot_to_excel):
    ...
    book = xlsxwriter.Workbook(excel_file_path,{'nan_inf_to_errors': True})#'cpk.xlsx'
    ...
    if p_val_checked == '1':
        report_sheet_title = [u'No', u'Item_name',u'BC',u'P_Val',u'BM',u'a_Q',u'a_irr',u'3CV',
...
def append_hash_to_excel(table_data,excel_file_path,hashfile_path):
    ...
    PostProgressMsg(60,"append_hash_to_excel ongoing","Excel report")
    ...
    #send_progress_info("append_hash_to_excel finish",80)
    PostProgressMsg(80,"append_hash_to_excel ongoing","Excel report")
    ...
    PostProgressMsg(100,"append_hash_to_excel finish","Excel report")
...
def create_report(event,header_list,df,color_by1,pic_path,select_category1,cpk_lsl,cpk_usl,
    ...
    send_progress_info("Excel report generating ...",60)
        ...
        if len(column_data) >0:   
            ...
            if stdev == 0 or stdev == 'nan':
                ...
                if p_val_checked == '1':
                    if event=='excel-report' and excel_report_item == 'all':#'Fail'
                        row_data = [j + 1, item_name,bc,p_val,BMC,a_Q,a_irr,three_CV,
                ...
                if cpk == None:
                    ...
                    if p_val_checked == '1':
                        if event=='excel-report' and excel_report_item == 'all':#'Fail'
                            row_data = [str(j + 1), item_name,bc,p_val,BMC,a_Q,a_irr,
                    ...
                    if cpk < cpk_lsl or cpk > cpk_usl:
                        ...
                        if p_val_checked == '1':
                            row_data = [j + 1, item_name,bc,p_val,BMC,a_Q,a_irr,three_CV,
                                        ...
                                        stdev, cpl, cpu, cpk, result,'','','','','','','']
                            report_sheet.write_row(location,row_data,format_normal)
                            report_sheet.write(j+1,18,result,new_format_fail)
                        else:
                        ...
                        if excel_report_item == 'all' or event == 'keynote-report':#'Fail'
                            ...
                            if p_val_checked == '1':
                                row_data = [j + 1, item_name,bc,p_val,BMC,a_Q,a_irr,
                                            three_CV,lsl,target_value,usl, min_num, 
                                            mean, max_num, stdev, cpl, cpu, cpk, 
                                            result,'','','','','','','']
                                report_sheet.write_row(location,row_data,format_normal)
                                report_sheet.write(j+1,18,result,new_format_pass)
                                wr_excel = 'Yes'
                            else:
                                row_data = [j + 1, item_name,bc,a_Q,a_irr,three_CV,
                                            lsl,target_value,usl, min_num, mean, 
            ...
            #--------------------------------get new limit from csv end------------------------
                ...
                if stdev == 0 or stdev == 'nan':
                    ...
                    if event=='excel-report' and excel_report_item == 'all':#'Fail'
                        if p_val_checked == '1':
                            report_sheet.write(j+1,19,new_lsl_backup,format_highlight)
                            report_sheet.write(j+1,20,target_value,format_highlight)
                            report_sheet.write(j+1,21,new_usl_backup,format_highlight)
                            report_sheet.write(j+1,22,cpl,format_highlight)
                            report_sheet.write(j+1,23,cpu,format_highlight)
                            report_sheet.write(j+1,24,cpk,format_highlight)
                            report_sheet.write(j+1,25,result,format_highlight)
                        else:
                    ...
                    if cpk == None:
                        ...
                        if p_val_checked == '1':
                            ...
                            report_sheet.write_row(location,row_data,format_normal)
                            if old_result == 'Fail':
                                report_sheet.write(j+1,18,old_result,new_format_fail)
                            elif old_result == 'Pass':
                                report_sheet.write(j+1,18,old_result,new_format_pass)
                            report_sheet.write(j+1,19,new_lsl_backup,format_highlight)
                            report_sheet.write(j+1,20,target_value,format_highlight)
                            report_sheet.write(j+1,21,new_usl_backup,format_highlight)
                            report_sheet.write(j+1,22,cpl,format_highlight)
                            report_sheet.write(j+1,23,cpu,format_highlight)
                            report_sheet.write(j+1,24,cpk,format_highlight)
                            report_sheet.write(j+1,25,result,new_format_fail)
                        else:
                            row_data = [j + 1, item_name,bc,a_Q,a_irr,three_CV,
                        ...
                        if cpk < cpk_lsl or cpk > cpk_usl:
                            ...
                            if excel_report_item == 'all' or event == 'keynote-report':#'Fail'
                                ...
                                if p_val_checked == '1':
                                    ...
                                    report_sheet.write(j+1,23,cpu,format_highlight)
                                    report_sheet.write(j+1,24,cpk,format_highlight)
                                    report_sheet.write(j+1,25,result,new_format_pass)
            ...
            if p_val_checked == '1':
                report_sheet.write(j+1,26,reviewer,format_normal)
                report_sheet.write(j+1,27,review_date,format_normal)
                report_sheet.write(j+1,28,comment,format_normal)
            else:
...
def generate_excel(table_data):
    ...
    PostProgressMsg(70,"generate_excel ongoing","Excel report")
    ...
    PostProgressMsg(100,"generate_excel finish","Excel report")
...
def excel_hash_to_csv(excel_path,csv_hash_path):
    PostProgressMsg(70,"excel_hash_to_csv start","Excel report")
    data = pd.read_excel(excel_path,'ssh',index_col=0,keep_default_na=False)
    data.to_csv(csv_hash_path,encoding='utf-8')
    PostProgressMsg(100,"excel_hash_to_csv finish","Excel report")

def excel_limitupdate_to_csv(excel_path,csv_limit_path):
    try:
        #print("exception happend here {}".format(e))
        PostProgressMsg(70,"excel_limitupdate_to_csv start","Excel report")
        data = pd.read_excel(excel_path,'report',index_col=0,keep_default_na=False)
        data.to_csv(csv_limit_path,encoding='utf-8')
        PostProgressMsg(100,"excel_limitupdate_to_csv finish","Excel report")
    except Exception as e:
        print("exception check here {}".format(e))
    ...
    while True:
        try:
            ...
            if len(zmqMsg)>0:
                ...
                #send_progress_info("Excel Report Start ...",20)
                
                PostProgressMsg(10,"Excel report Start ({})".format(key),"Excel report")

                if key == 'generate_excel_sheet1_hash':
                    table_data = get_redis_data(key)
                    
                    PostProgressMsg(20,"get redis data finish","Excel report")
                    if len(table_data)>0:
                        append_hash_to_excel(table_data,excel_report_name,hash_csv_path)
                        for x in range(1,10):
                            PostProgressMsg(100 + x*3,"append_hash_to_excel finish","Excel report")
                    else:
                        print("---get generate_excel_sheet1_hash error")
                        with open(filelognamehash, 'w') as file_object:
                            file_object.write("Finished,create excel report error")
                        PostProgressMsg(30,"redis data is none","Excel report")
                    return
                    ...
                    if len(table_data)>0:
                        ...
                        for x in range(1,10):
                            PostProgressMsg(100 + x*3,"generate_excel finish","Excel report")
                    else:
                    ...
                    if len(table_data)>0:
                        ...
                        for x in range(1,10):
                            PostProgressMsg(100 + x*3,"excel_hash_to_csv finish","Excel report")
                    else:
                ...
                elif key == 'excel_limit_update_to_csv_report':
                    table_data = get_redis_data(key)
                    PostProgressMsg(20,"get redis data finish","Excel report")
                    if len(table_data)>0:
                        excel_path = table_data[0]
                        ...
                        for x in range(1,10):
                            PostProgressMsg(100 + x*3,"excel_limitupdate_to_csv finish","Excel report")
                    else:
                        print("---get excel_limit_update_to_csv report error")
                        PostProgressMsg(30,"redis data is none","Excel report")
            ...
            PostProgressMsg(100,"Exception happend {}".format(str(e)),"Excel report")
