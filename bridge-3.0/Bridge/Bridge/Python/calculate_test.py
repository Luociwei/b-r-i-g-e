...
def open_all_csv(event,all_csv_path,data_select,remove_fail):
    ...
    return header_list,df,param_item_start_index
            ...
            if len(zmqMsg)>0:
                ...
                if len(msg)>5:
                    if msg[0] == 'calculate-param':
                        ...
                        time1 = time.time()
                        header_list,df,param_item_start_index =  open_all_csv('calculate-param',all_csv_path,data_select,remove_fail)
                        time2 = time.time()
                        ...
                        print('cpk_technology_show function write log :',filelogname)
                        print('time:',time1,time2,time3,time4)
                       
                        with open(filelogname, 'a') as file_object:
                            file_object.write("PASS,calculate is finished")
                            print('>calulate_param write PASS finish !')
