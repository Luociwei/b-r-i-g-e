...
def calculate_yield_rate(all_csv_path,yield_rate_csv_path):
    ...
    print('yield rate calculate finisehd!')

def calculate_yield_rate_local(all_csv_path,yield_rate_csv_path):
    all_csv_path = os.path.join(all_csv_path+ '')
    tmp_lst = []
    with open(all_csv_path, 'r') as f:
        reader = csv.reader(f)
    ...
    pass_df=data_df[data_df['PASS/FAIL'].isin(['PASS'])]
    overall_pass_sn = list(set(list((pass_df['SerialNumber'].values.tolist()))))
    # print('total_pass_sn:',len(total_pass_sn),total_pass_sn)

    fail_df=data_df[data_df['PASS/FAIL'].isin(['FAIL'])]
    overall_fail_sn = list(set(list((fail_df['SerialNumber'].values.tolist()))))
    # print('total_fail_sn:',len(total_fail_sn),total_fail_sn)
    no_retest_pass_sn = [x for x in overall_pass_sn if x not in overall_fail_sn]  #在list1列表中而不在list2列表中
    no_retest_pass_sn = list(set(no_retest_pass_sn))
    ...
    for retest_sn in total_retest_sn:
        # print('retest_sn:',retest_sn)
        if retest_sn !='':
            start_time_l = list(data_df.loc[ data_df['SerialNumber'] == retest_sn, 'Test Start Time'].tolist())
            # print('start_time_l:',start_time_l)
            first_test_time = min(start_time_l)

            # print('first_test_time:',first_test_time)
            sn_status = list(data_df.loc[ data_df['Test Start Time'].isin([first_test_time]) & (data_df['SerialNumber'] == retest_sn), 'PASS/FAIL'].tolist())
            # print('first_test_time,sn_status:',first_test_time,sn_status)
            if sn_status == ['PASS']:
                total_temp_p_l = total_temp_p_l + [retest_sn]
            elif sn_status == ['FAIL']:
    ...
    with open(yield_rate_csv_path,'w') as f:
        writer = csv.writer(f)
        # writer.writerow(['Station_ID','Test Count','Fail','Pass','Retest Count','Retest Rate (%)','Yield Percentage (%)'])
        # writer.writerow(['Station ID','Input (SN_TOTAL)',' FAIL SN Qty','PASS SN Qty','RETEST SN Qty','Retest Rate (%)','First Pass Yield (%)','FAIL RATE (%)'])
        writer.writerow(['Station ID','Input (TOTAL)',' FAIL Qty','PASS Qty','RETEST Qty','Retest Rate (%)','FPY (%)','FAIL Rate (%)'])

        writer.writerow(['All station',test_count,fail_count,pass_count,retest_count,str(retest_rate)+'%',str(yield_percentage)+'%',str(fail_percentage)+'%'])


    station_id_l = list(set(data_df['Station_ID'].values.tolist()))
    # print('station_id_l:',len(station_id_l),station_id_l)
    for station_id in station_id_l:
        total_sn_by_station_id = list((data_df.loc[(data_df['Station_ID'] == station_id), 'SerialNumber'].tolist()))
        total_sn_by_station_id = list(set(total_sn_by_station_id))
        # print(station_id+' total_sn_by_station_id:',len(total_sn_by_station_id),total_sn_by_station_id)

        pass_sn_by_station_id = list((data_df.loc[(data_df['Station_ID'] == station_id) & (data_df['PASS/FAIL'].isin(['PASS'])), 'SerialNumber'].tolist()))
        pass_sn_by_station_id = list(set(pass_sn_by_station_id))
        # print('pass_sn_by_station_id:',len(pass_sn_by_station_id),pass_sn_by_station_id)

        fail_sn_by_station_id = list((data_df.loc[(data_df['Station_ID'] == station_id) & (data_df['PASS/FAIL'].isin(['FAIL'])), 'SerialNumber'].tolist()))
        fail_sn_by_station_id = list(set(fail_sn_by_station_id))
        # print(station_id,'fail_sn_by_station_id:',len(fail_sn_by_station_id),fail_sn_by_station_id)
        ...
        for retest_sn in retest_pass_sn_by_station_id:
            if retest_sn != '':
                ...
                # print('first_test_time',first_test_time)                
                sn_status = list(data_df.loc[ data_df['Test Start Time'].isin([first_test_time]) & (data_df['SerialNumber'] == retest_sn), 'PASS/FAIL'].tolist())
                retest_station_id = list(data_df.loc[ data_df['Test Start Time'].isin([first_test_time]) & (data_df['SerialNumber'] == retest_sn), 'Station_ID'].tolist())
        ...
        temp_true_fail_l=[]
        for temp_true_fail_sn in true_fail_sn_by_station_id:
            start_time_l = list(data_df.loc[ data_df['SerialNumber'] == temp_true_fail_sn, 'Test Start Time'].tolist())
            first_test_time = min(start_time_l)

            sn_station_id = list(data_df.loc[ data_df['Test Start Time'].isin([first_test_time]) & (data_df['SerialNumber'] == temp_true_fail_sn), 'Station_ID'].tolist())
            if station_id == sn_station_id[0]:
                temp_true_fail_l = temp_true_fail_l + [temp_true_fail_sn]            
        fail_count_by_station_id = len(temp_true_fail_l)
        ...
        temp_no_retest_p_sn_l = []
        for no_retest_p_sn in no_retest_pass_sn_by_station_id:
            start_time_l = list(data_df.loc[ data_df['SerialNumber'] == no_retest_p_sn, 'Test Start Time'].tolist())
            first_test_time = min(start_time_l)
            sn_station_id = list(data_df.loc[ data_df['Test Start Time'].isin([first_test_time]) & (data_df['SerialNumber'] == no_retest_p_sn), 'Station_ID'].tolist())
            # print('sn_station_id:',sn_station_id)
            if station_id == sn_station_id[0]:
                temp_no_retest_p_sn_l = temp_no_retest_p_sn_l + [no_retest_p_sn]
...
def correlation(message):
    print("this function is calculate......")
...
def run(n):
    while True:
        try:
            global filelogname
            print("wait for yield retest rate client ...")
            zmqMsg = socket.recv()
            socket.send(b'yield_rate_sendback')
            if len(zmqMsg)>0:
                keyMsg = zmqMsg.decode('utf-8')
                print("message from retest rate client:", keyMsg)
                msg =keyMsg.split("$$")
                if len(msg)>3:
                    if msg[0] == 'yield_rate-param':
                        all_csv_path = msg[1]
                        yield_rate_csv_Path = msg[2]
                        calculate_yield_rate(all_csv_path,yield_rate_csv_Path)
                        filelogname = msg[3]
                        with open(filelogname, 'w') as file_object:
                            file_object.write("Finished,retest rate is finished")

                            return

                    if msg[0] == 'yield_rate-param-local':
                        all_csv_path = msg[1]
                        yield_rate_csv_Path = msg[2]
                        calculate_yield_rate_local(all_csv_path,yield_rate_csv_Path)
                        filelogname = msg[3]
                        with open(filelogname, 'w') as file_object:
                            file_object.write("Finished,retest rate is finished")
                            return
            ...
            print('error yield retest rate:',e)
            if filelogname == '':
                filelogname = '/tmp/CPK_Log/temp/.logretest.txt'
            with open(filelogname, 'w') as file_object:
                file_object.write("Finished,retest rate is finished")
