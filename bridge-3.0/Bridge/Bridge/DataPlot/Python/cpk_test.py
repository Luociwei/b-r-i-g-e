...
def cpk_plot(table_data,zmqMsg):
    try:
        global filelogname
        filelcpknew = '/tmp/CPK_Log/temp/.cpknew.txt'
        ...
        if select_color_by >0 or select_color_by_right>0:  # color by choose
            ...
            draw_more_histogram(tb_data2,all_data,zmqSelectItems,item_name,lsl, usl, mean, 
                                max_num, min_num, stdev, x1, y1, cpu, cpl, cpk, 
                                pic_path,set_bins,start_time_first,start_time_last,
                                BMC,zoom_type,range_set_lsl,range_set_usl,lsl_orig,usl_orig)
            DrawBoxPlot('/tmp/CPK_Log/temp/cpkbox.png',tb_data2,item_name,"Box Plot")
            ...
            draw_histogram(tb_data,item_name,lsl, usl, mean, max_num, min_num, 
                            stdev, x1, y1, cpu, cpl, cpk, pic_path,set_bins,
                            start_time_first,start_time_last,BMC,zoom_type,
                            range_set_lsl,range_set_usl,lsl_orig,usl_orig)
...
def probability_distribution_extend_by_color(column_category_data_list,data,zmqItem,bins,margin,item_name,
    ...
    for category_data in column_category_data_list:
        ...
        try:
            ...
            print("y_tick_cure here 000 ??")
            if pdf_check == '1':
                category_pdf = color_dpf[x%10]

                print("y_tick_cure here 111 ??")
                # if lsl_orig =='' or lsl_orig =='NA':
                #     lsl_orig = min(category_data)
                # if usl_orig =='' or usl_orig =='NA':
                #     usl_orig = max(category_data)
...
def DrawBoxPlot(pic_path,data,label,title):
    if type(data[0]) == type([]):
        data = data[0]
    print("DrawBoxPlot 0:{}".format(data))
    box_1 = data
    labels= [label]
    plt.figure(figsize=(6,5))
    plt.title(title,fontsize=20)

    print("DrawBoxPlot 1:{}".format(data))
    plt.boxplot([box_1], labels = labels,boxprops = {'color':'orangered'})

    print("DrawBoxPlot 2:{}".format(data))
    plt.grid(linestyle=':',c='gray')  # 生成网格
    plt.savefig(pic_path,dpi=200)
    plt.draw()
    plt.close('all')
