...
def creat_excel_report_file(path,file,cpk_lsl,cpk_usl,event,fail_plot_to_excel):
    ...
    report_sheet_title = [u'No', u'Item_name',u'BC',u'a_Q',u'a_irr',u'3CV',u'ORIG LSL',u'Target', u'ORIG USL', u'Min', u'Mean',u'Max', u'Std',u'CPL', u'CPU', u'CPK',cpk_result, u'New LSL',u'New Target',u'New USL',u'New CPL',u'New CPU',u'New CPK','New '+cpk_result,u'Reviewer Name',u'Review Date',u'Comment']
...
def valid_column(test_item_name,column_list):
    ...
    if len(column_list)< 3:# column_list[0] == '0' and column_list[1] == '0' or column_list[0] == '1' and column_list[1] == '1' or column_list[0] == column_list[1] and  column_list[0] !='NA' or 
...
def get_coefficients(value_l):
    ...
    if abs(u1) == 0:
        ...
        three_CV = three_sigma*100/abs(u1)
        # print('three_CV:',three_CV)
        return str(round(bc,6)),str(round(p_val,6)),str(round(a_Q,6)),str(round(a_irr,6)),str(round(three_CV,6))
...
def probability_distribution_extend(data,bins,margin,item_name,lsl,usl,mean,max_num,min_num,stdev,x1,y1,cpu,cpl,cpk,pic_path,bins_l,bins_h,start_time_first,start_time_last,bmc,zoom_type='limit'):
    bins = sorted(bins)
    length = len(bins)
    ...
    if zoom_type =='data':
        ...
        if lsl =='' or lsl =='NA':
            plt.xlim(bins_l-range_value, float(max_num)*1.2)

        elif usl =='' or usl =='NA':
            plt.xlim(float(min_num)*0.8, bins_h+range_value)
...
def probability_distribution_extend_by_color(column_category_data_list,data,bins,margin,item_name,lsl,usl,mean,max_num,min_num,stdev,x1,y1,cpu,cpl,cpk,pic_path,bins_l,bins_h,start_time_first,start_time_last,zoom_type):
...
def write_hash_to_excel(file_path,sheet_name,cell_format):
    hash_csv = file_path + 'temp/data_hash.csv'
...
#     hash_csv = hashfile_path + 'temp/data_hash.csv'
#     if not os.path.exists(hash_csv):
#         return None,None
...
def draw_histogram(column_data,item_name,lsl,usl,mean,max_num,min_num,stdev,x1,y1,cpu,cpl,cpk,pic_path,set_bins,start_time_first,start_time_last,bmc,zoom_type):
    ...
    probability_distribution_extend(column_data,bins,0,item_name,lsl,usl,mean,max_num,min_num,stdev,x1,y1,cpu,cpl,cpk,pic_path,bins_l,bins_h,start_time_first,start_time_last,bmc,zoom_type)

    return True


def draw_more_histogram(column_category_data_list,column_data, item_name, lsl, usl, mean, max_num, min_num, stdev, x1, y1,cpu, cpl, cpk, pic_path,set_bins,start_time_first,start_time_last,zoom_type):
    """
    ...
    # print(len(column_data),'---->',min(column_data),max(column_data),bins)
    probability_distribution_extend_by_color(column_category_data_list,column_data,bins, 0, item_name, lsl, usl, mean, max_num, min_num, stdev, x1, y1, cpu, cpl, cpk, pic_path,bins_l,bins_h,start_time_first,start_time_last,zoom_type)
...
def create_report(event,header_list,df,color_by1,pic_path,select_category1,cpk_lsl,cpk_usl,save_all_cpk_path,set_bins,excel_name,project_code,build_stage,station_name,start_time_first,start_time_last,color_by2,select_category_l2,excel_report_item,fail_plot_to_excel,zoom_type,param_item_start_index):
    ...
    path=pic_path
    result='pass'
    # print('table_data length in create_report:',len(table_data))
        ...
        if len(column_data) >0:   
            ...
            if stdev == 0 or stdev == 'nan':
                ...
                if event=='excel-report' and excel_report_item == 'all':#'Fail'
                    row_data = [j + 1, item_name,bc,a_Q,a_irr,three_CV,lsl,target_value, usl, min_num, mean, max_num, stdev, cpl, cpu, cpk, result,'','','','','','','']
                ...
                if cpk == None:
                    ...
                    # print('target_value--->',lsl,usl,target_value)
                    if event=='excel-report' and excel_report_item == 'all':#'Fail'
                        row_data = [str(j + 1), item_name,bc,a_Q,a_irr,three_CV,lsl,target_value, usl, min_num, mean, max_num, stdev, cpl, cpu, cpk, result,'','','','','','','']
                        report_sheet.write_row(location,row_data,format_normal)
                        wr_excel = 'Yes'
            ...
            #--------------------------------get new limit from csv end------------------------
                ...
                if stdev == 0 or stdev == 'nan':
                    ...
                    if cpk == None:
                        ...
                        row_data = [j + 1, item_name,bc,a_Q,a_irr,three_CV,old_lsl,old_target_value, old_usl, old_min_num, old_mean, old_max_num, old_stdev, old_cpl, old_cpu, old_cpk, old_result,lsl,target_value,usl,cpl,cpu,cpk,result]
                        report_sheet.write_row(location,row_data,format_normal)
                        if old_result == 'Fail':
...
def local_csv_generate_excel(table_data):
    ...
    filelogname = cpk_path + '/temp/.excel.txt'
    filelognamehash = cpk_path + '/temp/.excel_hash.txt'

    all_csv_path = table_data[10] #'/Users/RyanGao/Desktop/loadFile/20200714/iPad_data/test_data.csv'
    data_select = 'all'
    remove_fail = 'yes'
    event = 'excel-report'

    start_time = datetime.datetime.now() 
    excel_report_file_name ='Limit_Update_'+str(excel_report_user)+'_'+str(project_name)+'_For_'+str(excel_report_stage)+'_'+datetime.datetime.now().strftime('%Y-%m-%d')+'.xlsx'#%Y-%m-%d_%H-%M-%S
...
if __name__ == '__main__':
    excel_path = '/Users/RyanGao/Desktop/CPK_Log/Limit_Update_www111_www_For_www_2020-09-19.xlsx'
    csv_limit_path = '/Users/RyanGao/Desktop/CPK_Log/wwww.csv'
