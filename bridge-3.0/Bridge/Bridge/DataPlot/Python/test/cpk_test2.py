...
def cpk_plot(table_data,zmqMsg):
    ...
    if len(tbdata)>1:  # color by choose
        tb_data_raw=[]  #取出数据
        for i, v in enumerate(i_start):  #因start_data和End_data成对出现
        ...
        if stdev == 0:
            ...
            draw_more_histogram(tb_data2,all_data,zmqItem,item_name,lsl, usl, mean, max_num, min_num, stdev, x1, y1, cpu, cpl, cpk, pic_path,set_bins,start_time_first,start_time_last)
            info = item_name+' ===multi=>>item cpk and plot draw finished!'
            print(info)
            return info
        ...
        if stdev == 0:
            ...
            #     os.makedirs(pic_path)
                # os.system('mkdir '+pic_path)
            pic_path = pic_path + image_name
            draw_histogram(tb_data,item_name,lsl, usl, mean, max_num, min_num, stdev, x1, y1, cpu, cpl, cpk, pic_path,set_bins,start_time_first,start_time_last)
            info = item_name+' ====>>item cpk and plot draw finished!'
            print(info)
            return info


def draw_more_histogram(column_category_data_list,column_data,zmqItem,item_name, lsl, usl, mean, max_num, min_num, stdev, x1, y1,cpu, cpl, cpk, pic_path,set_bins,start_time_first,start_time_last):
    """
    ...
    probability_distribution_extend_by_color(column_category_data_list,column_data,zmqItem,bins, 0, item_name, lsl, usl, mean, max_num, min_num, stdev, x1, y1, cpu, cpl, cpk, pic_path,bins_l,bins_h,start_time_first,start_time_last)

    return True

def probability_distribution_extend_by_color(column_category_data_list,data,zmqItem,bins,margin,item_name,lsl,usl,mean,max_num,min_num,stdev,x1,y1,cpu,cpl,cpk,pic_path,bins_l,bins_h,start_time_first,start_time_last):
    ...
    plt.text(bins_l+range/3, y_ticks*0.78, info, size=10, rotation=0.0, alpha=0.85,fontsize=8,ha="left",va="center",bbox=dict(boxstyle="round", ec=('royalblue'),linestyle='-.',lw=1, fc=('white'), ))
...
def run(n):
    while True:
        try:
            ...
            if len(zmqMsg)>0:
                print("message from cpk client:", zmqMsg.decode('utf-8'))
...
if __name__ == '__main__':
    zmqMsg = 'CWNJ_C02-2FAP-23_2_FCT##CWNJ_C02-2FAP-24_1_FCT##retest_all&remove_fail_yes'
    table_data = get_redis_data(zmqMsg)
    # print(table_data)
    cpk_plot(table_data,zmqMsg)
    ...
    # table_data = get_redis_data(zmqMsg)
    # if len(table_data)>0:
    #     cpk_plot(table_data,pic_path,zmqMsg)
    # else:
    #     print("---get data error")
