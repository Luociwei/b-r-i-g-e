...
def generate_retest_plot(df_data,title_name,pie_retest_csv_path,pic_path,n_start, n_end):
    ...
    if len(df_data)>0:
        ...
        # data = df['Retest_Count'].values
        # data2 = df['traffic2'].values
        ...
        # plt.plot('Date', 'Retest_Count', data=df, color=c[0], label='Air ====Traffic1',alpha=1)
        # plt.plot('date', 'traffic2', data=df, color=c[1], label='Air Traffic2',alpha=1)
        ...
        # # Annotate
        # for t, p in zip(trough_locations[::1], peak_locations[::]):
        #     # plt.text(x0,y0,'%.2f' % y0,fontdict={'fontsize':14})
...
def generateRetestCSV(all_csv_path,retest_csv_path,pie_retest_csv_path,fail_csv_path,header_item_path):
    ...
    with open(all_csv_path, 'r') as f:
        ...
        try:
            ...
            buildsummary = '/tmp/CPK_Log/retest/.buildsummary.txt'
            with open(buildsummary, 'w') as file_object:
                file_object.write("\"StartTime\" in csv format wrong\r\n")
            return e
        ...
        if len(data_df) <=0:
            buildsummary = '/tmp/CPK_Log/retest/.buildsummary.txt'
            with open(buildsummary, 'a+') as file_object:
                file_object.write("no \"SerialNumber\" in csv\r\n")
        ...
        keyword_list2 = ['FIXTURE_SETUP CHANNEL CHANNEL_ID','FIXTURE CHANNEL ID','FIXTURE INITIALIZATION SLOT_ID','FIXTURE RESET CALC FIXTURE_CHANNEL','HEAD ID','FIXTURE_CHANNEL CHANNEL CHANNEL_ID','FIXTURE CHANNEL CHANNEL_ID','CHANNEL ID','CHANNEL_ID','SLOT ID','SLOT_ID','FIXTURE GET SLOT_ID','SLOT-ID','GET HEAD ID','GET HEAD_ID']
            ...
            if len(zmqMsg)>0:
                ...
                if len(msg)>3:
                    if msg[0] == 'retest_plot':
                        ...
                        #title = 'Retest Pareto Chart for Top 10'
                        title = 'Retest Pareto Chart'
                        color = 'steelblue'
                        ...
                        print('>pareto_plot Pareto Chart for Top 10 Retest finished')
                        
                        #title = 'Fail Pareto Chart for Top 10'
                        title = 'Fail Pareto Chart'
                        color = 'pink'
                        ...
                        print('>pareto_plot Pareto Chart for Top 10 Fail finished')
